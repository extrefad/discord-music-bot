...
const tocar = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(opt =>
      opt.setName('musica')
        .setDescription('Nome, link do YouTube ou Spotify')
        .setRequired(true)
    ),
...
const pular = {
  ...
  async execute(interaction, client) {
    ...
    try {
      await queue.skip();
      interaction.reply('⏭️ Pulado: **' + title + '**');
    } catch {
      interaction.reply('⏭️ Fila finalizada!');
    }
  },
};
...
const embaralhar = {
  ...
  async execute(interaction, client) {
    ...
    if (!queue || queue.songs.length < 2) {
      return interaction.reply({ content: '❌ Não há músicas suficientes na fila.', ephemeral: true });
    }
    queue.shuffle();
