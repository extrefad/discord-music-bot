...
const parar = {
  data: new SlashCommandBuilder().setName('parar').setDescription('Para a música e desconecta'),
  async execute(interaction, client) {
    const queue = client.distube.getQueue(interaction.guildId);
    if (!queue) return interaction.reply({ content: '❌ Nenhuma música tocando.', ephemeral: true });
...
const volume = {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption(opt => opt.setName('nivel').setDescription('Volume de 0 a 100').setMinValue(0).setMaxValue(100).setRequired(true)),
  async execute(interaction, client) {
    const queue = client.distube.getQueue(interaction.guildId);
    if (!queue) return interaction.reply({ content: '❌ Nenhuma música tocando.', ephemeral: true });
